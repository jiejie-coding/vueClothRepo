<template>
  <Nav>
    <div class="ingoods">
      <div class="ingoods-wrapper">
        <div class="ingoods-search">
          <el-input/>
      ...
      <el-dialog
        v-if="editData"
        class="ingoods-dialog"
        title="编辑入库记录"
        :visible.sync="editShow"
        width="30%">
        <el-form ref="editForm" :rules="rule" :model="editData" label-width="100px" size="mini" class="addIngoods-form">
          <el-table-column fixed prop="goodsId" label="货品Id" min-width="150"/>
          <el-table-column fixed prop="goodsName" label="货名" min-width="150"/>
          <el-table-column prop="size" label="大小" min-width="150"/>
          <el-table-column prop="amount" label="数量" min-width="150"/>
...
export default class InGoods extends Vue {
  ...
  getIngoodss() {
    axios.get('http://localhost:9999/c/ingoods/list')
    .then((response) => {
  ...
  deleteRow(obj) {
    this.$confirm('确认删除').then(() => {
      const config = {
        method: 'delete',
        url: 'http://localhost:9999/c/ingoods/delete',
  ...
  confirmEdit() {
    this.$refs.editForm.validate((valid) => {
      if (valid) {
        axios.post('http://localhost:9999/c/ingoods/edit', this.editData).then(() => {
          const loading = this.$loading({
            text: '正在加载中......',
            spinner: 'el-icon-loading',
            background: 'rgba(0, 0, 0, 0.7)',
